...
class Local extends CI_Controller {
  ...
  public function upload($id_local) {
    ...
    }if ($this->local_model->alterar_local($data, $id_local)) {
 ...
 public function cadastro($ajax = false) {
  if($ajax==true){
    $data['titulo'] = 'Rascunho';
    $data['tipo'] = 'Rascunho';
    $data['status'] = 2;
    $data['data_cadastro'] = date('Y-m-d');
    $data['idusuario'] = $this->session->userdata('id');
    $id = $this->local_model->cadastra_local($data);
    if($id){
      echo $id;
    }
  }else{
...
public function rascunho($id = null) {
...
public function alterar($id = null) {
  ...
  if ($_POST) {
    ...
    $this->form_validation->set_rules('titulo_google', 'Titulo do Google', 'trim|required|min_length[10]');
    $this->form_validation->set_rules('titulo_facebook', 'Titulo do Facebook', 'trim|required|min_length[10]');
    $this->form_validation->set_rules('descricao_google', 'Descrição do Google', 'trim|required|min_length[40]');
    $this->form_validation->set_rules('descricao_facebook', 'Descrição do Facebook', 'trim|required|min_length[10]');
